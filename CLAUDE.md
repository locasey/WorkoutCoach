...
- `llm_service.py` - Multi-provider LLM abstraction (Gemini / OpenAI / Anthropic). All providers with configured API keys are initialized at startup. `MODEL_TIERS` maps user role → model per provider; `_dispatch()` routes each call to the correct provider API based on model ID. `generate_workout_plan()` and `generate_periodized_workouts()` accept `user_role` to select the appropriate model. `generate_periodized_workouts()` also accepts `context_prompt: str` which is prepended to the LLM prompt (injected by `coach_context`). `get_models_for_role(user_role)` returns the full catalog with `available` flags for the UI model selector.
- `coach_context.py` - Builds a structured athlete context snapshot from the DB and renders it as a formatted string for LLM injection. `build_context_snapshot(db, user_id, week_offset=0)` returns a dict with: active training block info, current phase + week-in-phase, last 2 weeks of workout history (type, distance, completion status, notes, actuals), user preferences, and a `strava_data: None` reserved slot. `render_context_prompt(snapshot)` formats the snapshot as a ~140-token string injected before every LLM generation call. Returns `""` gracefully in maintenance mode or on any error.
...
- `training_block_service.py` - TrainingBlock CRUD, week context calculation, block overview with phase status, `regenerate_week()` (snapshot + delete + LLM regen for a single week). Accepts `user_preferences: dict` to resolve `experience_level` (no longer hardcoded) and injects athlete context via `coach_context`. `get_block_by_id()` accepts `user_id`.
- `periodized_workout_service.py` - Orchestrates phase-by-phase LLM workout generation for training blocks. Accepts `user_preferences: dict` to resolve `experience_level` from stored prefs. Builds athlete context snapshot once before the phase loop and injects it into each phase's LLM call.
